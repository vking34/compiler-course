...
Token* readNumber(void) {
  ...
  if (countPeriod == 0 || countPeriod == -1){
    ...
    token->tokenType = TK_NUMBERF;
    token->string[count++] = '0';
    token->string[count] = '\0';
    token->value = atof(token->string);
  } else error(ERR_INVALID_SYMBOL, token->lineNo, token->colNo);
  return token;
}
...
int readSpecialChar(){
  if (charCodes[currentChar] == CHAR_BACKSLASH){
    ...
    case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '0':
      while ((count < 3 && currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT)) {
	str[count++] = (char)currentChar;
	readChar();
      }
      str[count] = '\0';
      char* ptr;
      long oct = strtol(str, &ptr, 8);
      if (oct == -1 || count != 3) {
	jump(-count);
	return '\0';
      }
...
Token* readConstChar(void) {
  ...
  if (charCodes[currentChar] == CHAR_BACKSLASH)    
    token->string[0] = (char)readSpecialChar();
  else
...
Token* readString(void){
  ...
  while( currentChar != EOF && (charCodes[currentChar] !=  CHAR_DOUBLEQUOTE)){
      ...
      if (charCodes[currentChar] == CHAR_BACKSLASH){
	int c = readSpecialChar();
	if (c != -1) token->string[count++] = (char) c;
      }
      else {
	token->string[count++] = (char) currentChar;	
      }
...
void printToken(Token *token) {
  ...
  case TK_NUMBERF: printf("TK_NUMBERF(%s)\n", token->string); break;
